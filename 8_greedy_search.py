...
def greedy_search(inicio, meta, grafo):
    cola = deque() #Lista
    ...
    while cola:
        ...
        for arista in grafo(actual):
            costo_heuristico = grafo(arista[0], "Heuristic")

            if costo_heuristico < mejor_costo and arista[0] not in visitados: #Se busca el mejor costo total
                mejor_costo = costo_heuristico
...
inicio = 'A'
meta = 'H'
resultado = greedy_search(inicio, meta, data)
print(resultado)
